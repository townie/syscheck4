...
describe TextWithAnalytics do
  ...
  let(:big_text) do 'This This    a  paragraph without
                    duplication is is is execpt
                    This the frist a letters
                    This zebra apple crazy'
                 end
  ...
  it 'Number of each letter' do
    expect(smaller.letter_frequency).to eq('w' => 1, 'o' => 2, 't' => 1,
                                           'R' => 1, 'u' => 1, 'b' => 1,
                                           'y' => 1, 'r' => 1, 'U' => 1,
                                           'l' => 1, 'e' => 1, 's' => 1)
  end

  it 'Number of each symbol' do
    expect(smaller.symbol_frequency).to eq(' ' => 2, '!' => 1)
  end

  it 'Top three most common words' do
    expect(big.top_three).to eq(%w(This is a))
  end

  it 'Top three most common letters' do
    expect(big.bottom_three).to eq(%w(apple crazy duplication))
  end

  it 'Number of words only used once'
  it 'All letters not used in the string'
end
